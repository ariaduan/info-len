...
if args.LOO == True:
	fileout1 = open('../corpora_and_texts_obtained/4gm_google_book_{}_merge_leaveone'.format(lang),'w',encoding = 'utf-8')
else:
	fileout1 = open('../corpora_and_texts_obtained/4gm_google_book_{}_merge'.format(lang),'w',encoding = 'utf-8')
...
for line in file:
	...
	line = line.decode('utf-8').split()
	if len(line) < 7 or int(line[4]) < 1960 or int(line[4]) > 2000:
		continue
	for w in line[:-3]:
		if value.match(w):
			w = w.lower()
	tmp_bg = line[0].split("_")[0] + '_' + line[1].split('_')[0] + '_' + line[2].split('_')[0]
	tmp_tg = line[3].split("_")[0]
	tmp_cnt = int(line[5])
	if tmp_tg not in ix:
...
if args.LOO == True:
	inp = '../corpora_and_texts_obtained/4gm_google_book_{}_merge_leaveone'.format(lang)
else:
	inp = '../corpora_and_texts_obtained/4gm_google_book_{}_merge'.format(lang)
file1 = open(inp, 'rb')
