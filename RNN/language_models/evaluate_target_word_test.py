...
#for w in prefix:
#    print('dictionary.idx2word[w.item()]', dictionary.idx2word[w.item()])
# try auto-generate
...
if args.surprisalmode:
    ...
    with open(args.outf, 'w') as outf:
        for sentence in sentences:
            ...
            #outf.write(dictionary.idx2word[firstword.item()] + " ")
            output, hidden = model(input,hidden)  #can I see the model? To check what's the difference between it and mine      
            word_weights = output.squeeze().div(args.temperature).exp().cpu()
            word_surprisals = -1*torch.log2(word_weights/sum(word_weights))#[-logP(wi|c)]
            for word in sentence[1:len(prefix)]:
                  word_surprisal = word_surprisals[word].item()
                  totalsurprisal = word_surprisal + totalsurprisal#for each sentence
                  outf.write(dictionary.idx2word[word.item()] + " " + str(word_surprisal) + "\n")
                  input.fill_(word.item())
                  output, hidden = model(input, hidden)
                  word_weights = output.squeeze().div(args.temperature).exp().cpu()
                  word_surprisals = -1*torch.log2(word_weights/sum(word_weights))
            #outf.write("Total sentence surprisal (sans first word): " + str(totalsurprisal) + "\n")
