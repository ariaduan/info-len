...
p = ArgumentParser()
p.add_argument("input_folder", type = Path) # '/om/data/public/corpora/google_web_ngrams/data/4gms'
...
if args.LOO == True:
	fileout1 = open('../corpora_and_texts_obtained/4gm_{}_merge_50000_leaveone'.format(lang),'w',encoding = 'utf-8')
else:
	fileout1 = open('../corpora_and_texts_obtained/4gm_{}_merge_50000'.format(lang),'w',encoding = 'utf-8')
		...
		if i in replication_word_list:
			...
			for tmp_b in b:
				if b[tmp_b] - 1 == 0 or bg[tmp_b] - 1 == 0:
					continue
				ix[i][info] += (-m.log((b[tmp_b] - 1) / (bg[tmp_b] - 1), 2) * b[tmp_b] / 1000000)
	...
	for i in ix:
		if i in replication_word_list:
			...
			for tmp_b in b:
				if b[tmp_b] - 1 == 0 or bg[tmp_b] - 1 == 0:
					continue
				ix[i][info] += (-m.log(b[tmp_b] / bg[tmp_b], 2) * b[tmp_b] / 1000000)
...
if args.LOO == True:
	inp = '../corpora_and_texts_obtained/4gm_{}_merge_50000_leaveone'.format(lang)
else:
	inp = '../corpora_and_texts_obtained/4gm_{}_merge_50000'.format(lang)
file1 = open(inp, 'rb')
